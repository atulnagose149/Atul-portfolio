...
const Navbar = () => {
  ...
  const onClickLinkHandler = React.useCallback(
  ...
  return (
    <nav
      ref={ref}
      className="z-[999] relative flex items-center justify-center"
    >
      <div
        className={`fixed flex pt-28 lg:pt-36 pb-4 lg:pb-8 gap-3 w-full justify-between items-center px-10 sm:px-16 md:px-28 lg:px-44 xl:px-60 2xl:px-84 backdrop-blur-[3rem] lg:backdrop-blur-[0.3rem]`}
      >
        <motion.div
        ...
        >
          <Link href="#home">
            <Image
              src={name}
              alt={"Atul Nagose"}
              className="w-[18rem] object-contain cursor-pointer"
            />
          </Link>
        </motion.div>

        <motion.div
          className="flex gap-3 h-[3.25rem] items-center"
        ...
        >
                    ...
                    className={`flex w-full items-center justify-center px-3 py-2 my-1 transition 
                                        ${
                                          activeSection === link.name
                                            ? " text-gray-950 bg-gray-100 rounded-full dark:bg-gray-700 dark:text-gray-300 "
          ...
          {toggle ? (
            <BiX
              ...
              className="w-10 h-10 cursor-pointer bg-opacity-75 backdrop-blur-[0.5rem] active:scale-105 transition-all lg:hidden"
            />
          ) : (
            <BiMenu
              onClick={onClickToggleHandler}
        ...
        <motion.div
          ...
          } lg:hidden fixed mt-16 py-4 rounded-2xl self-start right-12 sm:right-16 md:right-28 max-h-[32rem] w-60 border-white border-opacity-40 bg-opacity-90 bg-white shadow-lg shadow-black/[0.05] px-3 dark:bg-gray-950 dark:border-black/40 dark:bg-opacity-90`}
            ...
            {links.map((link) => (
              ...
              >
                <Link
                  ...
                  onClick={onClickLinkHandler}
                  href={link.href}
                  className={`px-3 py-2 hover:text-gray-950 transition 
                                ${
                                  activeSection === link.name
                                    ? "text-gray-950 bg-gray-200 rounded-full dark:bg-gray-700 dark:text-gray-300"
                                    : ""
                                }`}
                >
                  {link.name}
